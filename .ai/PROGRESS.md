...
- [x] **GLX surface PROVEN reached + byte-faithful (post Mesa-injection, PR #599):** once a musl Mesa stack is in the image, glxtest dlopens libGL and issues GLX requests to the server (op=146: QueryVersion, GetVisualConfigs, GetFBConfigs, QueryServerString, ClientInfo). Replies are byte-correct on the wire (GetVisualConfigs seq reply_len=36/total=176 = 2 visuals × 18 props; GetFBConfigs reply_len=92/total=400 = 2 fbconfigs × 23 pairs × 2). The X11 GLX server is vindicated end-to-end.
- [x] **Mesa GL stack PROVEN working on our kernel (PSysEng GDB-autopsy verdict):** glxtest runs the real Mesa/llvmpipe **EGL** probe to completion and writes a correct 201-byte result to its pipe — `RENDERER\nllvmpipe (LLVM 17.0.6, 128 bits)\nVERSION\n4.5 ... Mesa 24.0.9 ... TEST_TYPE\nEGL` — then `exit_group(0)` clean. (An earlier "futex livelock" read of the serial was a misdiagnosis, refuted ~0.95: the `[FUTEX_WAKE_EXIT] woken=[]` lines are the normal 6×-per-process musl teardown signature; kernel futex counters all zero — faithful.)
- [ ] **Residual gate (latency, NOT kernel-logic):** gecko's parent reads the glxtest pipe with a single `poll(timeout=GFX_TEST_TIMEOUT=4000ms)` (compile-time, no env override). glxtest cold-demand-pages ~83 MB of Mesa+LLVM from ext2 (~13.4k block reqs, ~11.9k faults), exceeding 4 s, so the parent times out before glxtest's write/exit → "No GPUs detected" → FF falls back to software rendering and proceeds. **Non-blocking for a software-render demo** (llvmpipe is software anyway). If glxtest success is desired, beat the deadline by reducing Mesa cold-start demand-paging (readahead/prefetch the Mesa `.so`s in the VFS/ext2 read path + virtio-blk batching + page-cache residency — fs+mm+driver; or a toolchain pre-warm wrapper). Not a futex fix.
