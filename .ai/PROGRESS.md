...
- [ ] **NEW downstream gate (futex/threading, NOT X11):** glxtest then HANGS before CreateContext — the standalone `/disk/usr/lib/firefox-esr/glxtest` child freezes at sc=1904 while llvmpipe's worker thread pool deadlocks: 4 worker threads parked on their own per-thread futex uaddrs are never woken (`[FUTEX_WAKE_EXIT] ... woken=[]` repeating; wake targets the CLEARTID/thread-list addr, not the workers). FF's parent times out reading the glxtest pipe (`ManageChildProcess(glxtest): ... timeout`). This is the FUTEX_WAKE_GHOST / lost-wakeup family (Linux-personality futex/clone/thread-exit) — a separate investigation for the network/futex/principal specialists, bounded per the saga-exhaustion rule. Screendump confirms no FF paint (bare navy desktop).
