...
pub fn run() -> ! {
    ...
    // ── Test 142: chmod persists — stat reflects updated mode bits ────────

    total += 1;
    if test_chmod_persists() { passed += 1; }

    // ── Test 143: flock exclusive — second LOCK_EX|LOCK_NB returns -EWOULDBLOCK

    total += 1;
    if test_flock_exclusive_blocks_second() { passed += 1; }

    // ── Test 144: msync returns sane errno (ENOSYS) not silent 0 ─────────
