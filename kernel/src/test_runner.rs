...
pub fn run() -> ! {
    ...
    // ── Test 110: /dev/dsp open fails gracefully when AC97 absent ─────────

    total += 1;
    if test_dev_dsp_open_with_ac97_absent() { passed += 1; }

    // ── Test 111: /dev/dsp open + write when AC97 present (skip if absent) ─

    total += 1;
    if test_dev_dsp_open_with_ac97_present() { passed += 1; }

    // ── Test 112: /dev/dsp ioctl SNDCTL_DSP_SETFMT accepts S16_LE ─────────
