...
/// shootdown (0xF0), the diagnostic DR-sync IPI (0xF1), and the timer-wake poke
/// (0xF3); wired in `arch::x86_64::idt`.  Sent by [`resched_cpu`] to make a
/// remote CPU notice a freshly-runnable thread without waiting out a timer tick.
...
/// (Perf P2 phase 3c — the cross-CPU reschedule poke).
///
...
/// rather than waiting out its timer tick — closing the remote-wakeup latency
/// hole.  When `cpu` is the CURRENT CPU (always the case on SMP=1, where an
/// IPI-to-self would just be the local flag) no IPI is sent: setting the flag
...
/// a known starting state before exercising `resched_cpu`.
#[cfg(any(feature = "test-mode", feature = "firefox-test-core"))]
