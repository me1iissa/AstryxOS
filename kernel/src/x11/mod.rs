...
fn op_map_window(fd: u64, data: &[u8], seq: u16) {
    ...
    let wid = r32(data, 4);
    let mut to_paint: alloc::vec::Vec<u32> = alloc::vec::Vec::new();
    // Set true inside the closure when this map should claim the server-global
    // input focus (a top-level whose map drew a FocusIn).  The actual
    // `SERVER.focus_window` write happens AFTER the `with_client` block: that
    // block already holds the SERVER lock, and the lock is non-reentrant, so the
    // focus update cannot be done inside it.
    let mut claim_focus = false;
    with_client(fd, |c| {
        let (width, height, evmask, parent) = match c.resources.get_window_mut(wid) {
            Some(w) => {
                let dims = (w.width, w.height, w.event_mask, w.parent);
                w.mapped = true;
        ...
        let is_toplevel = parent == proto::ROOT_WINDOW_ID;
        ...
        // VisibilityChange: a window selecting VisibilityChangeMask is told its
        // visibility when it becomes viewable.  Per the X11 core protocol map
        // sequence this VisibilityNotify follows MapNotify and precedes Expose.
        // A freshly-mapped window with nothing above it is Unobscured (state 0).
        // This applies to any window (not just top-levels), so it is gated only
        // on the mask bit.
        if evmask & proto::EVENT_MASK_VISIBILITY_CHANGE != 0 {
            c.send(&event::encode_visibility_notify(seq, wid, 0));
        }
        ...
        // EnterNotify + FocusIn: a real (no-WM) X server, after exposing a
        // freshly-mapped top-level, delivers EnterNotify (the pointer is now in
        // the new surface) and FocusIn (it holds the input focus).  These are
        // TOP-LEVEL ONLY — a non-root/leaf child window must receive ONLY
        // MapNotify + Expose so toolkit leaf widgets (e.g. the xeyes Eyes child)
        // are not driven through a spurious crossing/focus pass.  Gated per-event
        // on the window's selected mask.  Pointer position is reported at the
        // window centre (root-relative for a top-level under the root).
        if is_toplevel {
            if evmask & proto::EVENT_MASK_ENTER_WINDOW != 0 {
                let cx = (width / 2) as i16;
                let cy = (height / 2) as i16;
                c.send(&event::encode_enter_notify(seq, wid, proto::ROOT_WINDOW_ID, cx, cy));
            }
            if evmask & proto::EVENT_MASK_FOCUS_CHANGE != 0 {
                c.send(&event::encode_focus_in(seq, wid));
                claim_focus = true;
            }
        }
        crate::serial_println!("[X11] MapWindow {:#x} {}x{}", wid, width, height);
    });
    // Claim the server-global input focus for the first top-level that drew a
    // FocusIn (focus defaults to the root window).  Done after with_client
    // releases the SERVER lock — the Mutex is non-reentrant.  Only the FIRST
    // top-level claims it so a later map does not steal focus from an
    // already-focused window.
    if claim_focus {
        let mut srv = SERVER.lock();
        if srv.focus_window == proto::ROOT_WINDOW_ID {
            srv.focus_window = wid;
        }
    }
