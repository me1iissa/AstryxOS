...
### Implication for the four tarballs in `SupportingResources/`
...
already have it via 2.43. The tarballs in `SupportingResources/OSCode/glibc/`
...
**REJECTED.** Out of scope; musl source not in `SupportingResources/`; would
...
No internal `SupportingResources/` paths, no "Linux kernel source", no
